  ...
  return (
    <main className="min-h-screen bg-gray-50 px-8 py-12">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Admin: Events</h1>
        ...
        <form onSubmit={onSubmit} className="bg-white border border-gray-100 rounded-xl p-6 space-y-4 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <select name="type" value={form.type} onChange={onChange} className="bg-gray-50 border border-gray-200 p-3 rounded-lg">
          ...
          <div className="flex gap-3">
            <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded-lg font-semibold">{editingId ? "Update" : "Add"} Event</button>
            {editingId && <button type="button" onClick={() => { setEditingId(""); setForm(initialForm); }} className="bg-gray-200 text-gray-900 px-6 py-2 rounded-lg font-semibold">Cancel</button>}
          </div>
        </form>
        ...
        <div className="space-y-4">
          {events.map((eventItem) => (
            <article key={eventItem.id} className="bg-white border border-gray-100 rounded-lg p-4">
              <h2 className="text-xl font-bold text-gray-900">{eventItem.title}</h2>
              <p className="text-sm text-gray-600">{eventItem.type} • {new Date(eventItem.eventDate).toLocaleString()}</p>
              <div className="mt-3 flex gap-3">
                <button type="button" onClick={() => startEdit(eventItem)} className="text-blue-700 font-semibold">Edit</button>
                <button type="button" onClick={() => remove(eventItem.id)} className="text-red-700 font-semibold">Delete</button>
              </div>
            </article>
          ))}
        </div>
